...
public class WorldMap : MonoBehaviour
{
    ...
    [SerializeField] DataTile wallDummyTile;
    ...
    public bool[,] notPassableTiles { get; private set; }
    ...
    public void GenerateWall(Vector2Int position, DataWall wallTile)
    {
        if (!IsInside(position)) return;
        int x = position.x;
        int y = position.y;
    ...
    public Item FindNearestItem(DataItem itemData, ItemClass itemClass, Vector2Int start)
    {
        if (!IsInside(start))
            return null;
    ...
    public Pawn CreatePawn(Vector2Int position, DataGenetics geneticData)
    {
    ...
    public void CreateBuilding(Vector2Int position, DataBuilding buildingObject, Direction direction)
    {
        if (buildingObject == null) return;
        if (!buildingObject.building.checkPlaceable(position, direction, this)) return;
        Building building = Instantiate(buildingObject.building);
        building.CurrentGridPosition = position;
        building.direction = direction;
    }

    public List<Item> CreateItem( Vector2Int position, DataItem itemData, ItemClass itemClass, int quantity, WorldObject reservingOb)
    {
        if (itemData == null)
            return null;
    ...
    Item GenerateItem(Vector2Int position, DataItem itemData, ItemClass itemClass, int quantity)
    {
