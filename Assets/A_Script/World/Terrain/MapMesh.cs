...
public class MapMesh
{
    ...
    public MapMesh(Map map, MapChunk chunk)
    {
        this.map = map;
        this.chunk = chunk;
    ...
    public void GenerateMesh()
    {
        ...
        int endX = Mathf.Min(startX + chunk.size.x, map.size.x);
        int endY = Mathf.Min(startY + chunk.size.y, map.size.y);

        for (int x = startX; x < endX; x++)
        {
            for (int y = startY; y < endY; y++)
            {
                CustomTile tile = map[x, y];
			    ...
			    */
                ...
                {   // We have a max of 8 neighbours.
                    ...
                    CustomTile neighbour = this.map[tile.position + ((TileDirection)i).Position()];
    ...
    public MeshData GetMesh(TerrainType terrainType)
    {
        ...
        {  // We already know this terrain type, let's return the mesh
